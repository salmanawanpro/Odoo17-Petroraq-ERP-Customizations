...
class HrLeave(models.Model):
    ...
    def get_absentees(self):
        ...
        if len(child_list) > 1:
            query = "SELECT employee_id,name,date_from,date_to FROM hr_leave " \
                    ...
                    "hr_employee.id WHERE hr_leave.state = 'validate' AND " \
                    "hr_leave.employee_id in %s" % str(tuple(child_list))
            self._cr.execute(query)
        elif len(child_list) == 1:
            query = "SELECT employee_id,name,date_from,date_to FROM hr_leave " \
                    "INNER JOIN hr_employee ON hr_leave.employee_id = " \
                    "hr_employee.id WHERE hr_leave.state = 'validate' AND " \
                    "hr_leave.employee_id = %s" % child_list[0]
            self._cr.execute(query)
    ...
    def get_upcoming_holidays(self):
        ...
        #         "resource_id is null"
        query = "SELECT * FROM public.hr_public_holiday WHERE " \
                "state = 'active'"
        self._cr.execute(query)
