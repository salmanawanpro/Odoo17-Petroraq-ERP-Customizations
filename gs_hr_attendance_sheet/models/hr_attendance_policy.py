...
class HrAttendancePolicy(models.Model):
    ...
    late_rule_id = fields.Many2one(comodel_name="hr.late.rule", required=True,
                                   string="Late In Rule")
    early_rule_id = fields.Many2one(comodel_name="hr.early.rule", required=True,
                                   string="Early Check Out Rule")
...
class HrEarlyRule(models.Model):
    _name = 'hr.early.rule'
    _description = 'Early Check Out Rules'

    name = fields.Char(string='name', required=True)
    line_ids = fields.One2many(comodel_name='hr.early.rule.line',
                               inverse_name='early_id', string='Early Check Out Periods')


class HrEarlyRuleLine(models.Model):
    _name = 'hr.early.rule.line'
    _description = 'Early Check Out Lines'
    type = [
        ('fix', 'Fixed'),
        ('rate', 'Rate')
    ]

    early_id = fields.Many2one(comodel_name='hr.early.rule', string='Early Rule')
    type = fields.Selection(string="Type", selection=type, required=True)
    rate = fields.Float(string='Rate')
    time = fields.Float('Time')
    amount = fields.Float('Amount')
